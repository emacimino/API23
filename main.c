...
void initStructure(station *firstStation) {
    ...
    firstStation->cars = calloc(MAX_CAR, sizeof(int));
...
void goBackRoute(int distance, int arrival, station *firstStation) {
    ...
    while (route->location != arrival) {
        ...
        //printf("%d ",goBackFreeway[tmpNumOfStation].location);
        tmpNumOfStation--;
        route = route->prev;
    }
    ...
    for (int tmpStation = numOfStations - 1; tmpStation > 0; tmpStation--) {
        ...
        for (int j = tmpStation - 1; j >= min_index; j--) {
            ...
            if (goBackFreeway[j].location - goBackFreeway[j].max_dis <= goBackFreeway[min_index - 1].location) {
                ...
                while (goBackFreeway[j].location - goBackFreeway[j].max_dis <= goBackFreeway[tmp_min_reach - 1].location && tmp_min_reach > 0) {
                    tmp_min_reach--;
                }
                if(goBackFreeway[j].location - goBackFreeway[j].max_dis <= goBackFreeway[tmp_min_reach].location)
                    passed = true;


                if (tmp_min_reach <= goBackFreeway[tmpStation].dijkstra_pred && passed) {
                    goBackFreeway[tmpStation].dijkstra_pred = j;
                    goBackFreeway[j].dijkstra_pred = tmp_min_reach;
                }
...
*/
int min_index = numOfStations - 1;
for(int tmpStation = numOfStations - 1; tmpStation > 0; tmpStation = min_index){
    int max_dis = goBackFreeway[tmpStation].location - goBackFreeway[tmpStation].max_dis;
    int min_reach = max_dis;
   for(int i = tmpStation - 1; goBackFreeway[i].location >= max_dis; i--){
       if(goBackFreeway[i].location - goBackFreeway[i].max_dis < max_dis){
           goBackFreeway[i].dijkstra_pred = tmpStation;
           if(goBackFreeway[i].location - goBackFreeway[i].max_dis < min_reach){
               min_reach = goBackFreeway[i].location - goBackFreeway[i].max_dis;
               min_index = i;
           }
       }
   }
   if(min_index == tmpStation){
       printf("nessun percorso\n");
       return;
   }
}
